...
// Configuration
$uploadDirectory = '../../../assets/images/uploads/banners/';
...
$activeTab = isset($_GET['tab']) ? $_GET['tab'] : 'privacy';
$editPolicyId = isset($_GET['edit']) ? (int)$_GET['edit'] : 0;
...
function handleImageUpload($fileInput, $uploadDir, $oldPath = null) {
    ...
    if (isset($_FILES[$fileInput]) && $_FILES[$fileInput]['error'] === UPLOAD_ERR_OK) {
        ...
        if (@move_uploaded_file($tempFile, $targetPath)) {
            ...
            if ($oldPath && strpos($oldPath, 'uploads/banners') !== false && file_exists('../../../' . $oldPath)) {
                @unlink('../../../' . $oldPath);
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_general_settings'])) {
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO privacy_policy_settings 
    ...
    elseif (isset($_POST['add_policy'])) {
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO privacy_policy (title, content, display_order, is_active) 
            ...
            $conn->rollBack();
            $message = "Error adding privacy policy section: " . $e->getMessage();
            $messageType = "error";
        }
    }
    
    // Update existing privacy policy section
    elseif (isset($_POST['update_policy'])) {
        try {
            $conn->beginTransaction();
            
            $id = (int)$_POST['policy_id'];
            $title = $_POST['title'] ?? '';
            $content = $_POST['content'] ?? '';
            $order = (int)$_POST['display_order'] ?? 0;
            $isActive = isset($_POST['is_active']) ? 1 : 0;
            
            $stmt = $conn->prepare("UPDATE privacy_policy 
                                 SET title = :title, 
                                     content = :content, 
            ...
            $messageType = "success";
            $activeTab = 'privacy';
            $editPolicyId = $id; // Keep editing the same section
        } catch (Exception $e) {
            $conn->rollBack();
            $message = "Error updating privacy policy section: " . $e->getMessage();
            $messageType = "error";
        }
    }
    ...
    elseif (isset($_POST['delete_policy'])) {
        try {
            ...
            $messageType = "success";
            $activeTab = 'privacy';
            $editPolicyId = 0; // Clear edit mode
        } catch (Exception $e) {
            $conn->rollBack();
            $message = "Error deleting privacy policy section: " . $e->getMessage();
            $messageType = "error";
        }
    }
    
    // Reorder privacy policy sections
    elseif (isset($_POST['reorder_policies'])) {
        try {
            $conn->beginTransaction();
            
            $policyIds = $_POST['policy_ids'] ?? [];
            $policyOrders = $_POST['policy_orders'] ?? [];
            
            if (count($policyIds) === count($policyOrders)) {
                for ($i = 0; $i < count($policyIds); $i++) {
                    $id = (int)$policyIds[$i];
                    $order = (int)$policyOrders[$i];
                    
                    $stmt = $conn->prepare("UPDATE privacy_policy SET display_order = :order WHERE id = :id");
                ...
                $conn->commit();
                $message = "Privacy policy order updated successfully!";
                $messageType = "success";
                $activeTab = 'privacy';
            } else {
            ...
            $conn->rollBack();
            $message = "Error reordering privacy policy: " . $e->getMessage();
            $messageType = "error";
        }
    }
}
...
$privacyPolicies = [];
try {
    $stmt = $conn->query("SELECT * FROM privacy_policy ORDER BY display_order ASC");
...
if ($editPolicyId > 0) {
    try {
        $stmt = $conn->prepare("SELECT * FROM privacy_policy WHERE id = :id");
        $stmt->bindParam(':id', $editPolicyId);
        $stmt->execute();
...
$nextOrder = 1;
if (!empty($privacyPolicies)) {
    $maxOrder = 0;
    foreach ($privacyPolicies as $policy) {
        if ((int)$policy['display_order'] > $maxOrder) {
            $maxOrder = (int)$policy['display_order'];
        }
    }
    ...
    <div class="min-h-screen flex flex-col lg:flex-row">
        ...
        <div class="flex-1 lg:ml-64">
            <div class="bg-white p-4 shadow-sm flex justify-between items-center">
                <h1 class="text-xl font-semibold text-gray-800">Edit Privacy Policy Page</h1>
                <div class="flex items-center space-x-4">
                    <a href="/privacy-policy" target="_blank" class="text-blue-600 hover:text-blue-800 flex items-center">
            ...
            <div class="p-6">
                ...
                <div class="mb-6 border-b border-gray-200">
                    <nav class="flex flex-wrap space-x-8">
                        <a href="?tab=privacy" class="py-4 px-1 border-b-2 font-medium text-sm leading-5 <?php echo $activeTab === 'privacy' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'; ?>">
                            Privacy Policy Sections
                        </a>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2">
                        <div class="bg-white rounded-lg shadow-md overflow-hidden">
                            <div class="p-6 border-b border-gray-200">
                                <h2 class="text-lg font-semibold text-gray-800">Privacy Policy Sections</h2>
                                <p class="text-sm text-gray-500 mt-1">Manage your privacy policy content</p>
                            </div>
                            ...
                            <div class="p-6">
                                ...
                                <div class="overflow-x-auto">
                                    <table class="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            <?php if (empty($privacyPolicies)): ?>
                                            <tr>
                                                <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">No privacy policy sections found. Add your first section.</td>
                                            </tr>
                                            <?php else: ?>
                                                <?php foreach ($privacyPolicies as $policy): ?>
                                                <tr>
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        <div class="text-sm font-medium text-gray-900"><?php echo htmlspecialchars($policy['title']); ?></div>
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $policy['is_active'] ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'; ?>">
                                                            <?php echo $policy['is_active'] ? 'Active' : 'Inactive'; ?>
                                                        </span>
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                        <?php echo (int)$policy['display_order']; ?>
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                        <a href="?edit=<?php echo $policy['id']; ?>" class="text-blue-600 hover:text-blue-900 mr-3">Edit</a>
                                                        <form method="POST" action="" class="inline-block">
                                                            <input type="hidden" name="policy_id" value="<?php echo $policy['id']; ?>">
                                                            <button type="submit" name="delete_policy" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to delete this section?');">Delete</button>
                                                        </form>
                                                    </td>
                                                </tr>
                                                <?php endforeach; ?>
                                ...
                                <!-- Reorder Form -->
                                <?php if (count($privacyPolicies) > 1): ?>
                                    ...
                                    <form method="POST" action="">
                                        <div class="space-y-3">
                                            ...
                                            <div class="flex items-center space-x-3">
                                                <input type="hidden" name="policy_ids[]" value="<?php echo $policy['id']; ?>">
                                                ...
                                                <div class="flex-1">
                                                    <span class="text-sm font-medium text-gray-700"><?php echo htmlspecialchars($policy['title']); ?></span>
                                                </div>
                                                <div class="w-20">
                                                    <input type="number" name="policy_orders[]" value="<?php echo (int)$policy['display_order']; ?>" min="1" 
                                        ...
                                        <div class="mt-4">
                                            <button type="submit" name="reorder_policies" 
                    ...
                    <!-- Add/Edit Privacy Policy Section Form -->
                    <div class="lg:col-span-1">
                        <div class="bg-white rounded-lg shadow-md overflow-hidden">
                            <div class="p-6 border-b border-gray-200">
                                <h2 class="text-lg font-semibold text-gray-800">
                                    <?php echo $editPolicy ? 'Edit Section' : 'Add New Section'; ?>
                                </h2>
                            </div>
                            ...
                            <div class="p-6">
                                <form method="POST" action="">
                                    ...
                                    <div class="space-y-6">
                                        ...
                                        <div class="mb-5">
                                            ...
                                            <input type="text" name="title" id="title" required
                                                value="<?php echo htmlspecialchars($editPolicy['title'] ?? ''); ?>"
                                        ...
                                        <div class="mb-5">
                                            ...
                                            <textarea name="content" id="content" rows="10" required
                                                class="block w-full px-4 py-2.5 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-300 focus:outline-none transition-colors duration-200 hover:border-gray-400"><?php echo htmlspecialchars($editPolicy['content'] ?? ''); ?></textarea>
                                        ...
                                        <div class="mb-5">
                                            ...
                                            <input type="number" name="display_order" id="display_order" min="1"
                                                value="<?php echo (int)($editPolicy['display_order'] ?? $nextOrder); ?>"
                                                class="block w-full px-4 py-2.5 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-300 focus:outline-none transition-colors duration-200 hover:border-gray-400">
                                        </div>
                                            ...
                                            <div class="flex items-start">
                                                <div class="flex items-center h-5">
                                                    <input type="checkbox" name="is_active" id="is_active"
                                                        <?php echo (!$editPolicy || $editPolicy['is_active']) ? 'checked' : ''; ?>
                                    ...
                                    <div class="mt-6 flex justify-end">
                                        <?php if ($editPolicy): ?>
                                        <a href="?tab=privacy" class="mr-3 inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
                                            Cancel
                                        </a>
                                        <button type="submit" name="update_policy" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
                                            Update Section
                                        </button>
                                        <?php else: ?>
                                        <button type="submit" name="add_policy" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
                                            Add Section
                                        </button>
                                        <?php endif; ?>
                                    </div>
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Customize the privacy policy page content and SEO</p>
                    </div>
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <input type="text" name="title" id="title" required
                                            ...
                                            class="block w-full px-4 py-2.5 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-300 focus:outline-none transition-colors duration-200 hover:border-gray-400">
                                        <p class="text-xs text-gray-500 mt-1">Main heading displayed on the privacy policy page</p>
                                    </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <input type="text" name="subtitle" id="subtitle"
                                            value="<?php echo htmlspecialchars($pageSettings['subtitle'] ?? 'Kebijakan Privasi'); ?>"
                                            class="block w-full px-4 py-2.5 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-300 focus:outline-none transition-colors duration-200 hover:border-gray-400">
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <input type="text" name="seo_title" id="seo_title"
                                            value="<?php echo htmlspecialchars($pageSettings['seo_title'] ?? 'Kebijakan Privasi | Akademi Merdeka'); ?>"
                                            class="block w-full px-4 py-2.5 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-300 focus:outline-none transition-colors duration-200 hover:border-gray-400">
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="inner_title" id="inner_title" required
                                        value="<?php echo htmlspecialchars($pageSettings['inner_title'] ?? 'Kebijakan Privasi'); ?>"
                                        class="block w-full px-4 py-2.5 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-300 focus:outline-none transition-colors duration-200 hover:border-gray-400">
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <input type="text" name="breadcrumb_current" id="breadcrumb_current"
                                            value="<?php echo htmlspecialchars($pageSettings['breadcrumb_current'] ?? 'Kebijakan Privasi'); ?>"
                                            class="block w-full px-4 py-2.5 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-300 focus:outline-none transition-colors duration-200 hover:border-gray-400">
                                    </div>
                                    
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-500">Preview: <span class="text-gray-700">Home > <?php echo htmlspecialchars($pageSettings['breadcrumb_parent'] ?? 'Tentang'); ?> > <?php echo htmlspecialchars($pageSettings['breadcrumb_current'] ?? 'Kebijakan Privasi'); ?></span></p>
                                    </div>
                                </div>
